...
class PharmsalesreturnitemExport implements FromCollection, WithMapping, WithHeadings
{
    use Exportable;
    ...
    {
        return [
            $sales->pharmacyproduct->name,
            $sales->pharmsalesreturn->patient->phone,
            $sales->pharmsalesentryitem->pharmpurchaseentryitem->batch,
            Carbon::parse($sales->expiry_date)->format('d-m-Y'),
            $sales->return_quantity,
            $sales->created_at->format('d-m-Y h:i A'),
            $sales->pharmsalesreturn->creatable->name,
        ];
    }
