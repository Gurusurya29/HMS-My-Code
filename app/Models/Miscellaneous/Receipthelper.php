...
class Receipthelper extends Model
{

    public static function autogeneratedreceiptid($digit, $prefix, $model)
    {
        if ($digit) {
            $uniqueId = Receipthelper::getNextSequencereceiptId($digit, $prefix, $model);
    ...
    public static function getNextSequencereceiptId($digit, $name, $model)
    {
        ...
        switch ($model->receipt_type) {
            case '1':
                ...
                $arr['hms_uniqid'] = 'ENH/H/R' . '-' . sprintf('%0' . $digit . 'd', intval($hms_lastId) + 1) . '/' . $year . '-' . $year + 1;
            ...
            case '4':
                ...
                $arr['pharm_uniqid'] = 'ENH/P/R' . '-' . sprintf('%0' . $digit . 'd', intval($pharm_lastId) + 1) . '/' . $year . '-' . $year + 1;
            ...
            case '5':
                ...
                $arr['lab_uniqid'] = 'ENH/L/R' . '-' . sprintf('%0' . $digit . 'd', intval($lab_lastId) + 1) . '/' . $year . '-' . $year + 1;
            ...
            case '6':
                ...
                $arr['scan_uniqid'] = 'ENH/S/R' . '-' . sprintf('%0' . $digit . 'd', intval($scan_lastId) + 1) . '/' . $year . '-' . $year + 1;
            ...
            case '7':
                ...
                $arr['xray_uniqid'] = 'ENH/X/R' . '-' . sprintf('%0' . $digit . 'd', intval($xray_lastId) + 1) . '/' . $year . '-' . $year + 1;
                $arr['xray_sequence_id'] = $xray_lastId + 1;
                break;

            default:
                Log::info("Unknown Receipt sequence");
                Log::info("Receipt" . $model);
                break;
        }
