...
class Investigationreceiptlivewire extends Component
{
    use datatableLivewireTrait, miscellaneousLivewireTrait;

    public $user, $patient, $balance, $total_amount, $searchquery, $patientlist = [], $receipt_type_data;
    public $payment_type = 1, $receipt_type, $received_amount, $modeofpayment, $payment_ref_id, $bank_name, $payment_date,
        $note;
    ...
    {
        $this->user = auth()->guard('laboratory')->user();
        $this->receipt_type_data = collect(config('archive.receipt_type'))->where('maintype', 'Investigation')->toArray();
    ...
    public function store()
    {
        ...
        try {
            ...
            Helper::trackmessage($this->user, $receipt, 'inverstiagation_receiptcreateoredit', session()->getId(), 'WEB', 'Inverstiagation Receipt payment');

            $this->toaster('success', 'Receipt Paid Successfully!!');
            DB::commit();
            $this->printreceiptentry($receipt);
            return redirect()->route('investigationreceipt');
        } catch (Exception $e) {
            $this->exceptionerror($this->user, 'inverstiagation_receipt_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'inverstiagation_receipt_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'inverstiagation_receipt_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.laboratory.receipt.investigationreceiptlivewire');
    }
}
