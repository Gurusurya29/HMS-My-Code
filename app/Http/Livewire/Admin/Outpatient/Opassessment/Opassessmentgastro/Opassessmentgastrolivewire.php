...
namespace App\Http\Livewire\Admin\Outpatient\Opassessment\Opassessmentgastro;
...
class Opassessmentgastrolivewire extends Component
{
    ...
    public function store()
    {
        ...
        if ($this->labinvestigation_file) {
            $validatedData['labinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->labinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Gastro\Opgastro', 'labinvestigation_file');
        ...
        if ($this->scaninvestigation_file) {
            $validatedData['scaninvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->scaninvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Gastro\Opgastro', 'scaninvestigation_file');
        ...
        if ($this->xrayinvestigation_file) {
            $validatedData['xrayinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->xrayinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Gastro\Opgastro', 'xrayinvestigation_file');
        ...
        if ($this->prescription_file) {
            $validatedData['prescription_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->prescription_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Gastro\Opgastro', 'prescription_file');
        ...
        try {
            ...
            if ($this->outpatient->specialable) {
                $opgastro = $this->outpatient->specialable;
                $opgastro->update($validatedData);
                $this->user->opgastroupdatable()->save($opgastro);

                $this->multiselectsync($opgastro);
                ...
                Helper::trackmessage($this->user, $opgastro, 'patientassesment_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
                ...
                $this->outpatient->specialable()->associate($opgastro)
                    ->save();
                if ($this->is_movetoip) {
                    $inpatient = $this->user->inpatientcreatable()
                        ->create([
                ...
                Helper::trackmessage($this->user, $opgastro, 'patient_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
    ...
    protected function multiselectsync($opgastro)
    {
        $opgastro->currentcomplaints()->sync($this->currentcomplaint);
        $opgastro->physicalexam()->sync($this->physicalexam);
        $opgastro->diagnosismaster()->sync($this->diagnosismaster);

        $this->labinvestigationsync($this->labinvestigation, $this->user, $this->outpatient->patient_id, $opgastro, 'OP', 'OP Gastro', $this->is_labinvestigationemergency);
        $this->scaninvestigationsync($this->scaninvestigation, $this->user, $this->outpatient->patient_id, $opgastro, 'OP', 'OP Gastro', $this->is_scaninvestigationemergency);
        $this->xrayinvestigationsync($this->xrayinvestigation, $this->user, $this->outpatient->patient_id, $opgastro, 'OP', 'OP Gastro', $this->is_xrayinvestigationemergency);
    }
    ...
    {
        return view('livewire.admin.outpatient.opassessment.opassessmentgastro.opassessmentgastrolivewire');
    }
}
