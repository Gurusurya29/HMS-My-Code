...
namespace App\Http\Livewire\Admin\Outpatient\Opassessment\Opassessmentgeneralsurgeon;
...
class Opassessmentgeneralsurgeonlivewire extends Component
{
    use WithFileUploads;
    use UploadTrait;
    ...
    public function store()
    {
        ...
        if ($this->labinvestigation_file) {
            $validatedData['labinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->labinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Generalsurgeon\Opgeneralsurgeon', 'labinvestigation_file');
        ...
        if ($this->scaninvestigation_file) {
            $validatedData['scaninvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->scaninvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Generalsurgeon\Opgeneralsurgeon', 'scaninvestigation_file');
        ...
        if ($this->xrayinvestigation_file) {
            $validatedData['xrayinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->xrayinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Generalsurgeon\Opgeneralsurgeon', 'xrayinvestigation_file');
        ...
        if ($this->prescription_file) {
            $validatedData['prescription_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->prescription_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Generalsurgeon\Opgeneralsurgeon', 'prescription_file');
        ...
        try {
            ...
            if ($this->outpatient->specialable) {
                $opgeneralsurgeon = $this->outpatient->specialable;
                $opgeneralsurgeon->update($validatedData);
                $this->user->opgeneralsurgeonupdatable()->save($opgeneralsurgeon);

                $this->multiselectsync($opgeneralsurgeon);
                ...
                Helper::trackmessage($this->user, $opgeneralsurgeon, 'patientassesment_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
                ...
                $opgeneralsurgeon = $this->user
                    ->opgeneralsurgeoncreatable()
                ...
                $this->outpatient->specialable()->associate($opgeneralsurgeon)
                    ->save();
                if ($this->is_movetoip) {
                    $inpatient = $this->user->inpatientcreatable()
                        ->create([
                ...
                Helper::trackmessage($this->user, $opgeneralsurgeon, 'patient_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
    ...
    protected function multiselectsync($opgeneralsurgeon)
    {
        $opgeneralsurgeon->currentcomplaints()->sync($this->currentcomplaint);
        $opgeneralsurgeon->physicalexam()->sync($this->physicalexam);
        $opgeneralsurgeon->diagnosismaster()->sync($this->diagnosismaster);

        $this->labinvestigationsync($this->labinvestigation, $this->user, $this->outpatient->patient_id, $opgeneralsurgeon, 'OP', 'OP General Surgeon', $this->is_labinvestigationemergency);
        $this->scaninvestigationsync($this->scaninvestigation, $this->user, $this->outpatient->patient_id, $opgeneralsurgeon, 'OP', 'OP General Surgeon', $this->is_scaninvestigationemergency);
        $this->xrayinvestigationsync($this->xrayinvestigation, $this->user, $this->outpatient->patient_id, $opgeneralsurgeon, 'OP', 'OP General Surgeon', $this->is_xrayinvestigationemergency);
    }
    ...
    public function formreset()
    {
        ...
        $this->systemicexamfinding_note = $this->dietadvice_note = $this->nextvisit_date = $this->doctor_note = null;
    ...
    {
        return view('livewire.admin.outpatient.opassessment.opassessmentgeneralsurgeon.opassessmentgeneralsurgeonlivewire');
    }
}
