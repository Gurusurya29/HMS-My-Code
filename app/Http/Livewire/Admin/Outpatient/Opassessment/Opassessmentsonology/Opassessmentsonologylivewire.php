...
namespace App\Http\Livewire\Admin\Outpatient\Opassessment\Opassessmentsonology;
...
class Opassessmentsonologylivewire extends Component
{
    ...
    public function store()
    {
        ...
        if ($this->labinvestigation_file) {
            $validatedData['labinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->labinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Sonology\Opsonology', 'labinvestigation_file');
        ...
        if ($this->scaninvestigation_file) {
            $validatedData['scaninvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->scaninvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Sonology\Opsonology', 'scaninvestigation_file');
        ...
        if ($this->xrayinvestigation_file) {
            $validatedData['xrayinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->xrayinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Sonology\Opsonology', 'xrayinvestigation_file');
        ...
        if ($this->prescription_file) {
            $validatedData['prescription_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->prescription_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Sonology\Opsonology', 'prescription_file');
        ...
        try {
            ...
            if ($this->outpatient->specialable) {
                $opsonology = $this->outpatient->specialable;
                $opsonology->update($validatedData);
                $this->user->opsonologyupdatable()->save($opsonology);

                $this->multiselectsync($opsonology);
                ...
                Helper::trackmessage($this->user, $opsonology, 'patientassesment_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
                ...
                $this->outpatient->specialable()->associate($opsonology)
                    ->save();
                if ($this->is_movetoip) {
                    $inpatient = $this->user->inpatientcreatable()
                        ->create([
                ...
                Helper::trackmessage($this->user, $opsonology, 'patient_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
    ...
    protected function multiselectsync($opsonology)
    {
        $opsonology->currentcomplaints()->sync($this->currentcomplaint);
        $opsonology->physicalexam()->sync($this->physicalexam);
        $opsonology->diagnosismaster()->sync($this->diagnosismaster);

        $this->labinvestigationsync($this->labinvestigation, $this->user, $this->outpatient->patient_id, $opsonology, 'OP', 'OP Sonology', $this->is_labinvestigationemergency);
        $this->scaninvestigationsync($this->scaninvestigation, $this->user, $this->outpatient->patient_id, $opsonology, 'OP', 'OP Sonology', $this->is_scaninvestigationemergency);
        $this->xrayinvestigationsync($this->xrayinvestigation, $this->user, $this->outpatient->patient_id, $opsonology, 'OP', 'OP Sonology', $this->is_xrayinvestigationemergency);
    }
    ...
    {
        return view('livewire.admin.outpatient.opassessment.opassessmentsonology.opassessmentsonologylivewire');
    }
}
