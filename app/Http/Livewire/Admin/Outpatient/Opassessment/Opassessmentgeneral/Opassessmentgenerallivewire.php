...
namespace App\Http\Livewire\Admin\Outpatient\Opassessment\Opassessmentgeneral;
...
use PDOException;
class Opassessmentgenerallivewire extends Component
{
    use WithFileUploads;
    use UploadTrait;
    ...
    public function store()
    {
        ...
        if ($this->labinvestigation_file) {
            $validatedData['labinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->labinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\General\Opgeneral', 'labinvestigation_file');
        ...
        if ($this->scaninvestigation_file) {
            $validatedData['scaninvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->scaninvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\General\Opgeneral', 'scaninvestigation_file');
        ...
        if ($this->xrayinvestigation_file) {
            $validatedData['xrayinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->xrayinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\General\Opgeneral', 'xrayinvestigation_file');
        ...
        if ($this->prescription_file) {
            $validatedData['prescription_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->prescription_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\General\Opgeneral', 'prescription_file');
        ...
        try {
            ...
            if ($this->outpatient->specialable) {
                $opgeneral = $this->outpatient->specialable;
                $opgeneral->update($validatedData);
                $this->user->opgeneralupdatable()->save($opgeneral);

                $this->multiselectsync($opgeneral);
                ...
                Helper::trackmessage($this->user, $opgeneral, 'patientassesment_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
                ...
                $this->outpatient->specialable()->associate($opgeneral)
                    ->save();
                if ($this->is_movetoip) {
                    $inpatient = $this->user->inpatientcreatable()
                        ->create([
                ...
                Helper::trackmessage($this->user, $opgeneral, 'patient_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
    ...
    protected function multiselectsync($opgeneral)
    {
        $opgeneral->currentcomplaints()->sync($this->currentcomplaint);
        $opgeneral->physicalexam()->sync($this->physicalexam);
        $opgeneral->diagnosismaster()->sync($this->diagnosismaster);

        $this->labinvestigationsync($this->labinvestigation, $this->user, $this->outpatient->patient_id, $opgeneral, 'OP', 'OP General', $this->is_labinvestigationemergency);
        $this->scaninvestigationsync($this->scaninvestigation, $this->user, $this->outpatient->patient_id, $opgeneral, 'OP', 'OP General', $this->is_scaninvestigationemergency);
        $this->xrayinvestigationsync($this->xrayinvestigation, $this->user, $this->outpatient->patient_id, $opgeneral, 'OP', 'OP General', $this->is_xrayinvestigationemergency);
    }
    ...
    {
        return view('livewire.admin.outpatient.opassessment.opassessmentgeneral.opassessmentgenerallivewire');
    }
}
