...
namespace App\Http\Livewire\Admin\Outpatient\Opassessment\Opassessmentneurology;
...
class Opassessmentneurologylivewire extends Component
{
    ...
    public function store()
    {
        ...
        if ($this->labinvestigation_file) {
            $validatedData['labinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->labinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Neurology\Opneurology', 'labinvestigation_file');
        ...
        if ($this->scaninvestigation_file) {
            $validatedData['scaninvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->scaninvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Neurology\Opneurology', 'scaninvestigation_file');
        ...
        if ($this->xrayinvestigation_file) {
            $validatedData['xrayinvestigation_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->xrayinvestigation_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Neurology\Opneurology', 'xrayinvestigation_file');
        ...
        if ($this->prescription_file) {
            $validatedData['prescription_file'] = $this->uploadfile($this->outpatient->specialable ? $this->outpatient->specialable->id : null, $this->prescription_file, '/admin/outpatient/' . $this->outpatient->uniqid, 'App\Models\Admin\Outpatient\Neurology\Opneurology', 'prescription_file');
        ...
        try {
            ...
            if ($this->outpatient->specialable) {
                $opneurology = $this->outpatient->specialable;
                $opneurology->update($validatedData);
                $this->user->opneurologyupdatable()->save($opneurology);

                $this->multiselectsync($opneurology);
                ...
                Helper::trackmessage($this->user, $opneurology, 'patientassesment_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
                ...
                $opneurology = $this->user
                    ->opneurologycreatable()
                ...
                $this->outpatient->specialable()->associate($opneurology)
                    ->save();
                if ($this->is_movetoip) {
                    $inpatient = $this->user->inpatientcreatable()
                        ->create([
                ...
                Helper::trackmessage($this->user, $opneurology, 'patient_createoredit', session()->getId(), 'WEB', 'Patient Assessment Created');
    ...
    protected function multiselectsync($opneurology)
    {
        $opneurology->currentcomplaints()->sync($this->currentcomplaint);
        $opneurology->physicalexam()->sync($this->physicalexam);
        $opneurology->diagnosismaster()->sync($this->diagnosismaster);

        $this->labinvestigationsync($this->labinvestigation, $this->user, $this->outpatient->patient_id, $opneurology, 'OP', 'OP Neurology', $this->is_labinvestigationemergency);
        $this->scaninvestigationsync($this->scaninvestigation, $this->user, $this->outpatient->patient_id, $opneurology, 'OP', 'OP Neurology', $this->is_scaninvestigationemergency);
        $this->xrayinvestigationsync($this->xrayinvestigation, $this->user, $this->outpatient->patient_id, $opneurology, 'OP', 'OP Neurology', $this->is_xrayinvestigationemergency);
    }
    ...
    {
        return view('livewire.admin.outpatient.opassessment.opassessmentneurology.opassessmentneurologylivewire');
    }
}
