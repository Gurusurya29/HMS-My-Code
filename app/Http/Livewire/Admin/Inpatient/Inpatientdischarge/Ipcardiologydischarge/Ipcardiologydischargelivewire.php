...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipcardiologydischarge;
...
class Ipcardiologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dscardiologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dscardiologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dscardiologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dscardiologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dscardiologydata->is_billpaid;
            $this->principaldiagnosis = $this->dscardiologydata->principaldiagnosis;
            $this->riskfactor = $this->dscardiologydata->riskfactor;
            $this->cheifcomplaint = $this->dscardiologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dscardiologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dscardiologydata->historyofpastillness;
            $this->others = $this->dscardiologydata->others;
            $this->hospitalizationcourse = $this->dscardiologydata->hospitalizationcourse;
            $this->operativesummary = $this->dscardiologydata->operativesummary;
            $this->conditionatdischarge = $this->dscardiologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dscardiologydata->casesheet_file;
            $this->specialinstruction = $this->dscardiologydata->specialinstruction;
            $this->physicalexamination = $this->dscardiologydata->physicalexamination;
            $this->prescription_note = $this->dscardiologydata->prescription_note;
            $this->discharge_date = $this->dscardiologydata->discharge_date;
            $this->is_patientdischarged = $this->dscardiologydata->is_patientdischarged;

            if ($this->dscardiologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dscardiologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dscardiologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dscardiologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dscardiologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dscardiology\Dscardiology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dscardiologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dscardiology = $this->dscardiologydata;
                $dscardiology->update($validatedData);
                $this->user->dscardiologyupdatable()->save($dscardiology);

                if ($dscardiology->subprescriptionable) {
                    $dscardiology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dscardiology->subprescriptionable);
                } else {
                    ...
                    $dscardiology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dscardiology, 'dscardiology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dscardiology = $this->user->dscardiologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dscardiology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dscardiology, 'dscardiology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dscardiology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dscardiology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dscardiology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipcardiologydischarge.ipcardiologydischargelivewire');
    }
}
