...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ippaediatricdischarge;
...
class Ippaediatricdischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dspaediatricdata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dspaediatricdata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dspaediatricdata->doctor_id;
            $this->dischargeinitiate_note = $this->dspaediatricdata->dischargeinitiate_note;
            $this->is_billpaid = $this->dspaediatricdata->is_billpaid;
            $this->principaldiagnosis = $this->dspaediatricdata->principaldiagnosis;
            $this->riskfactor = $this->dspaediatricdata->riskfactor;
            $this->cheifcomplaint = $this->dspaediatricdata->cheifcomplaint;
            $this->historyofpresentillness = $this->dspaediatricdata->historyofpresentillness;
            $this->historyofpastillness = $this->dspaediatricdata->historyofpastillness;
            $this->others = $this->dspaediatricdata->others;
            $this->hospitalizationcourse = $this->dspaediatricdata->hospitalizationcourse;
            $this->operativesummary = $this->dspaediatricdata->operativesummary;
            $this->conditionatdischarge = $this->dspaediatricdata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dspaediatricdata->casesheet_file;
            $this->specialinstruction = $this->dspaediatricdata->specialinstruction;
            $this->physicalexamination = $this->dspaediatricdata->physicalexamination;
            $this->prescription_note = $this->dspaediatricdata->prescription_note;
            $this->discharge_date = $this->dspaediatricdata->discharge_date;
            $this->is_patientdischarged = $this->dspaediatricdata->is_patientdischarged;

            if ($this->dspaediatricdata->subprescriptionable) {
                $prescriptionlist_data = $this->dspaediatricdata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dspaediatricdata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dspaediatricdata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dspaediatricdata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dspaediatric\Dspaediatric', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dspaediatricdata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dspaediatric = $this->dspaediatricdata;
                $dspaediatric->update($validatedData);
                $this->user->dspaediatricupdatable()->save($dspaediatric);

                if ($dspaediatric->subprescriptionable) {
                    $dspaediatric->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dspaediatric->subprescriptionable);
                } else {
                    ...
                    $dspaediatric->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dspaediatric, 'dspaediatric_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dspaediatric = $this->user->dspaediatriccreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dspaediatric)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dspaediatric, 'dspaediatric_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dspaediatric_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dspaediatric_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dspaediatric_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ippaediatricdischarge.ippaediatricdischargelivewire');
    }
}
