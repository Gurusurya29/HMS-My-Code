...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipnephrologydischarge;
...
class Ipnephrologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsnephrologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsnephrologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsnephrologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dsnephrologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsnephrologydata->is_billpaid;
            $this->principaldiagnosis = $this->dsnephrologydata->principaldiagnosis;
            $this->treatment = $this->dsnephrologydata->treatment;
            $this->riskfactor = $this->dsnephrologydata->riskfactor;
            $this->cheifcomplaint = $this->dsnephrologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsnephrologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dsnephrologydata->historyofpastillness;
            $this->others = $this->dsnephrologydata->others;
            $this->hospitalizationcourse = $this->dsnephrologydata->hospitalizationcourse;
            $this->operativesummary = $this->dsnephrologydata->operativesummary;
            $this->conditionatdischarge = $this->dsnephrologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsnephrologydata->casesheet_file;
            $this->specialinstruction = $this->dsnephrologydata->specialinstruction;
            $this->physicalexamination = $this->dsnephrologydata->physicalexamination;
            $this->prescription_note = $this->dsnephrologydata->prescription_note;
            $this->discharge_date = $this->dsnephrologydata->discharge_date;
            $this->is_patientdischarged = $this->dsnephrologydata->is_patientdischarged;

            if ($this->dsnephrologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dsnephrologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsnephrologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsnephrologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsnephrologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsnephrology\Dsnephrology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsnephrologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsnephrology = $this->dsnephrologydata;
                $dsnephrology->update($validatedData);
                $this->user->dsnephrologyupdatable()->save($dsnephrology);

                if ($dsnephrology->subprescriptionable) {
                    $dsnephrology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsnephrology->subprescriptionable);
                } else {
                    ...
                    $dsnephrology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsnephrology, 'dsnephrology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsnephrology = $this->user->dsnephrologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsnephrology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsnephrology, 'dsnephrology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsnephrology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsnephrology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsnephrology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipnephrologydischarge.ipnephrologydischargelivewire');
    }
}
