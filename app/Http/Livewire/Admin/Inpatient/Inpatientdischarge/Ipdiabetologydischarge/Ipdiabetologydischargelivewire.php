...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipdiabetologydischarge;
...
class Ipdiabetologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsdiabetologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsdiabetologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsdiabetologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dsdiabetologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsdiabetologydata->is_billpaid;
            $this->principaldiagnosis = $this->dsdiabetologydata->principaldiagnosis;
            $this->riskfactor = $this->dsdiabetologydata->riskfactor;
            $this->cheifcomplaint = $this->dsdiabetologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsdiabetologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dsdiabetologydata->historyofpastillness;
            $this->others = $this->dsdiabetologydata->others;
            $this->hospitalizationcourse = $this->dsdiabetologydata->hospitalizationcourse;
            $this->operativesummary = $this->dsdiabetologydata->operativesummary;
            $this->conditionatdischarge = $this->dsdiabetologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsdiabetologydata->casesheet_file;
            $this->specialinstruction = $this->dsdiabetologydata->specialinstruction;
            $this->physicalexamination = $this->dsdiabetologydata->physicalexamination;
            $this->prescription_note = $this->dsdiabetologydata->prescription_note;
            $this->discharge_date = $this->dsdiabetologydata->discharge_date;
            $this->is_patientdischarged = $this->dsdiabetologydata->is_patientdischarged;

            if ($this->dsdiabetologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dsdiabetologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsdiabetologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsdiabetologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsdiabetologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsdiabetology\Dsdiabetology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsdiabetologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsdiabetology = $this->dsdiabetologydata;
                $dsdiabetology->update($validatedData);
                $this->user->dsdiabetologyupdatable()->save($dsdiabetology);

                if ($dsdiabetology->subprescriptionable) {
                    $dsdiabetology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsdiabetology->subprescriptionable);
                } else {
                    ...
                    $dsdiabetology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsdiabetology, 'dsdiabetology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsdiabetology = $this->user->dsdiabetologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsdiabetology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsdiabetology, 'dsdiabetology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsdiabetology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsdiabetology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsdiabetology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipdiabetologydischarge.ipdiabetologydischargelivewire');
    }
}
