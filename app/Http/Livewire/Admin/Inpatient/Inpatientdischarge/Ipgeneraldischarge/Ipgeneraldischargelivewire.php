...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipgeneraldischarge;
...
class Ipgeneraldischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsgeneraldata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsgeneraldata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsgeneraldata->doctor_id;
            $this->dischargeinitiate_note = $this->dsgeneraldata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsgeneraldata->is_billpaid;
            $this->principaldiagnosis = $this->dsgeneraldata->principaldiagnosis;
            $this->treatment = $this->dsgeneraldata->treatment;
            $this->riskfactor = $this->dsgeneraldata->riskfactor;
            $this->cheifcomplaint = $this->dsgeneraldata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsgeneraldata->historyofpresentillness;
            $this->historyofpastillness = $this->dsgeneraldata->historyofpastillness;
            $this->others = $this->dsgeneraldata->others;
            $this->hospitalizationcourse = $this->dsgeneraldata->hospitalizationcourse;
            $this->operativesummary = $this->dsgeneraldata->operativesummary;
            $this->conditionatdischarge = $this->dsgeneraldata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsgeneraldata->casesheet_file;

            $this->specialinstruction = $this->dsgeneraldata->specialinstruction;
            $this->physicalexamination = $this->dsgeneraldata->physicalexamination;
            $this->prescription_note = $this->dsgeneraldata->prescription_note;
            $this->discharge_date = $this->dsgeneraldata->discharge_date;
            $this->is_patientdischarged = $this->dsgeneraldata->is_patientdischarged;

            if ($this->dsgeneraldata->subprescriptionable) {
                $prescriptionlist_data = $this->dsgeneraldata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsgeneraldata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsgeneraldata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsgeneraldata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsgeneral\Dsgeneral', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsgeneraldata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsgeneral = $this->dsgeneraldata;
                $dsgeneral->update($validatedData);
                $this->user->dsgeneralupdatable()->save($dsgeneral);

                if ($dsgeneral->subprescriptionable) {
                    $dsgeneral->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsgeneral->subprescriptionable);
                } else {
                    $prescription = $this->user->prescriptioncreatable()->create([
                        ...
                        'subtype' => 'General Discharge Summary',
                    ]);
                    $this->inpatient->prescriptionable()->save($prescription);
                    $dsgeneral->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsgeneral, 'dsgeneral_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsgeneral = $this->user->dsgeneralcreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsgeneral)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsgeneral, 'dsgeneral_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsgeneral_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsgeneral_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsgeneral_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipgeneraldischarge.ipgeneraldischargelivewire');
    }
}
