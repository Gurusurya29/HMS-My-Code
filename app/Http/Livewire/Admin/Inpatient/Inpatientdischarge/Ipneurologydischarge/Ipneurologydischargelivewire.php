...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipneurologydischarge;
...
class Ipneurologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsneurologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsneurologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsneurologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dsneurologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsneurologydata->is_billpaid;
            $this->principaldiagnosis = $this->dsneurologydata->principaldiagnosis;
            $this->riskfactor = $this->dsneurologydata->riskfactor;
            $this->cheifcomplaint = $this->dsneurologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsneurologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dsneurologydata->historyofpastillness;
            $this->others = $this->dsneurologydata->others;
            $this->hospitalizationcourse = $this->dsneurologydata->hospitalizationcourse;
            $this->operativesummary = $this->dsneurologydata->operativesummary;
            $this->conditionatdischarge = $this->dsneurologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsneurologydata->casesheet_file;
            $this->specialinstruction = $this->dsneurologydata->specialinstruction;
            $this->physicalexamination = $this->dsneurologydata->physicalexamination;
            $this->prescription_note = $this->dsneurologydata->prescription_note;
            $this->discharge_date = $this->dsneurologydata->discharge_date;
            $this->is_patientdischarged = $this->dsneurologydata->is_patientdischarged;

            if ($this->dsneurologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dsneurologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsneurologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsneurologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsneurologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsneurology\Dsneurology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsneurologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsneurology = $this->dsneurologydata;
                $dsneurology->update($validatedData);
                $this->user->dsneurologyupdatable()->save($dsneurology);

                if ($dsneurology->subprescriptionable) {
                    $dsneurology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsneurology->subprescriptionable);
                } else {
                    ...
                    $dsneurology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsneurology, 'dsneurology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsneurology = $this->user->dsneurologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsneurology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsneurology, 'dsneurology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsneurology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsneurology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsneurology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipneurologydischarge.ipneurologydischargelivewire');
    }
}
