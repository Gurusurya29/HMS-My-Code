...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipophthalmologydischarge;
...
class Ipophthalmologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsophthalmologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsophthalmologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsophthalmologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dsophthalmologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsophthalmologydata->is_billpaid;
            $this->principaldiagnosis = $this->dsophthalmologydata->principaldiagnosis;
            $this->riskfactor = $this->dsophthalmologydata->riskfactor;
            $this->cheifcomplaint = $this->dsophthalmologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsophthalmologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dsophthalmologydata->historyofpastillness;
            $this->others = $this->dsophthalmologydata->others;
            $this->hospitalizationcourse = $this->dsophthalmologydata->hospitalizationcourse;
            $this->operativesummary = $this->dsophthalmologydata->operativesummary;
            $this->conditionatdischarge = $this->dsophthalmologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsophthalmologydata->casesheet_file;
            $this->specialinstruction = $this->dsophthalmologydata->specialinstruction;
            $this->physicalexamination = $this->dsophthalmologydata->physicalexamination;
            $this->prescription_note = $this->dsophthalmologydata->prescription_note;
            $this->discharge_date = $this->dsophthalmologydata->discharge_date;
            $this->is_patientdischarged = $this->dsophthalmologydata->is_patientdischarged;

            if ($this->dsophthalmologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dsophthalmologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsophthalmologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsophthalmologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsophthalmologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsophthalmology\Dsophthalmology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsophthalmologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsophthalmology = $this->dsophthalmologydata;
                $dsophthalmology->update($validatedData);
                $this->user->dsophthalmologyupdatable()->save($dsophthalmology);

                if ($dsophthalmology->subprescriptionable) {
                    $dsophthalmology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsophthalmology->subprescriptionable);
                } else {
                    ...
                    $dsophthalmology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsophthalmology, 'dsophthalmology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsophthalmology = $this->user->dsophthalmologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsophthalmology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsophthalmology, 'dsophthalmology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsophthalmology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsophthalmology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsophthalmology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipophthalmologydischarge.ipophthalmologydischargelivewire');
    }
}
