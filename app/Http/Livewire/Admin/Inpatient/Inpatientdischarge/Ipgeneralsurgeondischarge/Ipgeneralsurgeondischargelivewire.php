...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipgeneralsurgeondischarge;
...
class Ipgeneralsurgeondischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsgeneralsurgeondata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsgeneralsurgeondata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsgeneralsurgeondata->doctor_id;
            $this->dischargeinitiate_note = $this->dsgeneralsurgeondata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsgeneralsurgeondata->is_billpaid;
            $this->principaldiagnosis = $this->dsgeneralsurgeondata->principaldiagnosis;
            $this->treatment = $this->dsgeneralsurgeondata->treatment;
            $this->riskfactor = $this->dsgeneralsurgeondata->riskfactor;
            $this->cheifcomplaint = $this->dsgeneralsurgeondata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsgeneralsurgeondata->historyofpresentillness;
            $this->historyofpastillness = $this->dsgeneralsurgeondata->historyofpastillness;
            $this->others = $this->dsgeneralsurgeondata->others;
            $this->hospitalizationcourse = $this->dsgeneralsurgeondata->hospitalizationcourse;
            $this->operativesummary = $this->dsgeneralsurgeondata->operativesummary;
            $this->conditionatdischarge = $this->dsgeneralsurgeondata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsgeneralsurgeondata->casesheet_file;
            $this->specialinstruction = $this->dsgeneralsurgeondata->specialinstruction;
            $this->physicalexamination = $this->dsgeneralsurgeondata->physicalexamination;
            $this->prescription_note = $this->dsgeneralsurgeondata->prescription_note;
            $this->discharge_date = $this->dsgeneralsurgeondata->discharge_date;
            $this->is_patientdischarged = $this->dsgeneralsurgeondata->is_patientdischarged;

            if ($this->dsgeneralsurgeondata->subprescriptionable) {
                $prescriptionlist_data = $this->dsgeneralsurgeondata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsgeneralsurgeondata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsgeneralsurgeondata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    {
        $this->dipatch('loaddoctorSelect2Hydrate');
    }
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsgeneralsurgeondata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsgeneralsurgeon\Dsgeneralsurgeon', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsgeneralsurgeondata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsgeneralsurgeon = $this->dsgeneralsurgeondata;
                $dsgeneralsurgeon->update($validatedData);
                $this->user->dsgeneralsurgeonupdatable()->save($dsgeneralsurgeon);

                if ($dsgeneralsurgeon->subprescriptionable) {
                    $dsgeneralsurgeon->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsgeneralsurgeon->subprescriptionable);
                } else {
                    $prescription = $this->user->prescriptioncreatable()->create([
                        ...
                        'subtype' => 'General Surgeon Discharge Summary',
                    ]);
                    $this->inpatient->prescriptionable()->save($prescription);
                    $dsgeneralsurgeon->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsgeneralsurgeon, 'dsgeneralsurgeon_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsgeneralsurgeon = $this->user->dsgeneralsurgeoncreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsgeneralsurgeon)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsgeneralsurgeon, 'dsgeneralsurgeon_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsgeneralsurgeon_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsgeneralsurgeon_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsgeneralsurgeon_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        $this->dipatch('printdischargesummary', $inpatient->id);
    }

    public function render()
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipgeneralsurgeondischarge.ipgeneralsurgeondischargelivewire');
    }
}
