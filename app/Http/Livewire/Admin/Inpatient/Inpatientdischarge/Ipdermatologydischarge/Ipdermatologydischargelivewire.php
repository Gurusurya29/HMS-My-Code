...
namespace App\Http\Livewire\Admin\Inpatient\Inpatientdischarge\Ipdermatologydischarge;
...
class Ipdermatologydischargelivewire extends Component
{
    use miscellaneousLivewireTrait;
    use WithFileUploads;
    use UploadTrait;

    public $inpatient, $doctorlist, $dsdermatologydata, $user;
    ...
    public function mount($inpatient_uuid)
    {
        ...
        if ($this->inpatient->dsspecialable) {
            $this->dsdermatologydata = $this->inpatient->dsspecialable;
            $this->doctor_id = $this->dsdermatologydata->doctor_id;
            $this->dischargeinitiate_note = $this->dsdermatologydata->dischargeinitiate_note;
            $this->is_billpaid = $this->dsdermatologydata->is_billpaid;
            $this->principaldiagnosis = $this->dsdermatologydata->principaldiagnosis;
            $this->riskfactor = $this->dsdermatologydata->riskfactor;
            $this->cheifcomplaint = $this->dsdermatologydata->cheifcomplaint;
            $this->historyofpresentillness = $this->dsdermatologydata->historyofpresentillness;
            $this->historyofpastillness = $this->dsdermatologydata->historyofpastillness;
            $this->others = $this->dsdermatologydata->others;
            $this->hospitalizationcourse = $this->dsdermatologydata->hospitalizationcourse;
            $this->operativesummary = $this->dsdermatologydata->operativesummary;
            $this->conditionatdischarge = $this->dsdermatologydata->conditionatdischarge;
            $this->tempcasesheet_file = $this->dsdermatologydata->casesheet_file;
            $this->specialinstruction = $this->dsdermatologydata->specialinstruction;
            $this->physicalexamination = $this->dsdermatologydata->physicalexamination;
            $this->prescription_note = $this->dsdermatologydata->prescription_note;
            $this->discharge_date = $this->dsdermatologydata->discharge_date;
            $this->is_patientdischarged = $this->dsdermatologydata->is_patientdischarged;

            if ($this->dsdermatologydata->subprescriptionable) {
                $prescriptionlist_data = $this->dsdermatologydata->subprescriptionable->prescriptionlist;
                $arr = [];
            ...
            if ($this->dsdermatologydata->followupvisit) {
                $arr = [];
                foreach (json_decode($this->dsdermatologydata->followupvisit) as $key => $value) {
                    array_push($arr, [
    ...
    public function store()
    {
        ...
        if ($this->casesheet_file) {
            $validatedData['casesheet_file'] = $this->uploadfile($this->dsdermatologydata->id ?? null, $this->casesheet_file, '/admin/ipassesment/' . $this->inpatient->uniqid, 'App\Models\Admin\Dischargesummary\Dsdermatology\Dsdermatology', 'casesheet_file');
        } else {
            $validatedData['casesheet_file'] = $this->tempcasesheet_file;
        }
        try {
            DB::beginTransaction();
            if ($this->dsdermatologydata) {
                $validatedData['followupvisit'] = json_encode($validatedData['followupvisit']);
                $dsdermatology = $this->dsdermatologydata;
                $dsdermatology->update($validatedData);
                $this->user->dsdermatologyupdatable()->save($dsdermatology);

                if ($dsdermatology->subprescriptionable) {
                    $dsdermatology->subprescriptionable->prescriptionlist()->delete();
                    $this->prescriptioncreate($dsdermatology->subprescriptionable);
                } else {
                    $prescription = $this->user->prescriptioncreatable()->create([
                        ...
                        'subtype' => 'Dermatology Discharge Summary',
                    ]);
                    $this->inpatient->prescriptionable()->save($prescription);
                    $dsdermatology->subprescriptionable()->save($prescription);
                ...
                Helper::trackmessage($this->user, $dsdermatology, 'dsdermatology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Updated');
                ...
                $dsdermatology = $this->user->dsdermatologycreatable()
                    ->create($validatedData);
                $this->inpatient->dsspecialable()->associate($dsdermatology)
                    ->save();
                if ($this->is_patientdischarged) {
                    $this->inpatient->update([
                        'is_patientdischarged' => Carbon::now(),
                ...
                Helper::trackmessage($this->user, $dsdermatology, 'dsdermatology_createoredit', session()->getId(), 'WEB', 'Discharge Summary Created');
            ...
            $this->exceptionerror($this->user, 'admin_dsdermatology_createoredit', 'error_one : ' . $e->getMessage());
        } catch (QueryException $e) {
            $this->exceptionerror($this->user, 'admin_dsdermatology_createoredit', 'error_two : ' . $e->getMessage());
        } catch (PDOException $e) {
            $this->exceptionerror($this->user, 'admin_dsdermatology_createoredit', 'error_three : ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.admin.inpatient.inpatientdischarge.ipdermatologydischarge.ipdermatologydischargelivewire');
    }
}
