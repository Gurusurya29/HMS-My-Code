...
<script>
    // 1. GLOBAL MODAL INITIALIZATION (Still necessary)
    const myModal = new bootstrap.Modal(document.getElementById('showModal'), {
        backdrop: 'static',
        keyboard: false
    });

    // 2. Custom Livewire Hook to show the modal *AFTER* the DOM is updated
    // This replaces the functionality of nextTick()
    Livewire.on('patientDataLoaded', () => {
        // Use a slight delay here to ensure everything is settled, then show the modal.
        setTimeout(() => {
            myModal.show();
        }, 50);
    });

    // Keep the rest of your print/close logic the same, using myModal.hide() and myModal.show()
