            ...
            <div class="row g-3">
                ...
                @if ($payment_to == 1 || $payment_to == 2 || $payment_to == 3)
                    <div class="col-md-4">
                        ...
                        <select wire:model.debounce.150ms="selecteduser_id" class="form-select" id="select2-dropdown" wire:ignore>
...
<script>
    document.addEventListener('livewire:init', () => {
        // Initialize Select2
        function loadPaymentUserSelect2() {
            ...
            select.off('change').on('change', function () {
                Livewire.find(select.closest('[wire\\:id]').attr('wire:id'))
                    .set('selecteduser_id', $(this).val());
            });
        }
        ...
        // When Livewire updates DOM (after re-render)
        Livewire.hook('morph.updated', () => {
            loadPaymentUserSelect2();
        });

        // Optional: if you emit a custom event from Livewire like → $this->dispatch('loadpaymentSelect2Hydrate')
        Livewire.on('loadpaymentSelect2Hydrate', () => {
            loadPaymentUserSelect2();
        });
    });
</script>
