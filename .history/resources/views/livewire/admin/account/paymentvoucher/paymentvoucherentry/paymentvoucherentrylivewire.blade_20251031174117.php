            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <!-- <select wire:model.lazy="selecteduser_id" class="form-select" id="select2-dropdown"> -->
                        <select wire:model.lazy="selecteduser_id" class="form-select" id="select2-dropdown" wire:ignore>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <span class="text-danger fw-bold">*</span>
                    <select class="form-select" wire:model.lazy="payment_type" wire:ignore>
...
<script>
    function initializeSelect2() {
        // Destroy existing Select2 instance before re-initialization
        $('#select2-dropdown').select2('destroy');

        $('#select2-dropdown').select2().on('change', function() {
            // Use @this.set for Livewire 2/3 compatibility
            @this.set('selecteduser_id', $(this).val());
        });

        // Ensure the initial value is selected
        $('#select2-dropdown').val(@this.get('selecteduser_id')).trigger('change');
    }
    ...
    // Hook into Livewire's DOM update cycle (for v2/v3)
    // This is crucial for when Livewire changes data but doesn't re-render the select due to wire:ignore
    Livewire.hook('element.updated', (el, component) => {
        if (el.id === 'select2-dropdown') {
            initializeSelect2();
        }
    });

    // Your existing listener for when $payment_to changes and the list updates
