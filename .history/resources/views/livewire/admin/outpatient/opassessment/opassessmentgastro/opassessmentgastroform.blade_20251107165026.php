    ...
    document.addEventListener('livewire:init', () => {
        ...
        const initSelect2Single = (el, property) => {
            ...
            $el.on('change.select2', function() {
                Livewire.find(compId)?.set(property, $(this).val(), false);
            });
        };

        const selectMappings = [
            { selector: '.drug-dropdown', property: 'selectedDrug' },
        ...
        const initAll = (root = document) => {
            selectMappings.forEach(({ selector, property }) => {
                $(root).find(selector).each(function() {
                    const $el = $(this);
                    setTimeout(() => initSelect2Single($el, property), 50);
                });
            });
        };
        ...
        Livewire.hook('morph.updated', ({ el }) => {
            requestAnimationFrame(() => {
                initAll(el instanceof Element ? el : document);
            });
        });
    });
</script>
