...
document.addEventListener('livewire:init', () => {
    ...
    /**
     * Initialize a single Select2 dropdown safely
     */
    const initSelect2 = ($el, property) => {
        ...
        // Detect if it's inside a modal (fixes dropdown position)
        ...
        // Sync Livewire → Select2 (retain values on render)
        try {
            const currentVal = Livewire.find(compId).get(property);
            if (currentVal !== undefined && currentVal !== null) {
                $el.val(Array.isArray(currentVal) ? currentVal.map(String) : String(currentVal))
                    .trigger('change.select2');
            }
        } catch (err) {
            console.warn(`Livewire value sync failed for ${property}`, err);
        }
    ...
    /**
     * Initialize all Select2 dropdowns on page
     */
    const initAllSelect2 = (root = document) => {
        ...
        selectMappings.forEach(({ selector, property }) => {
            $(root).find(selector).each(function () {
                ...
                // Wait for <option> tags to appear (useful if Livewire renders slowly)
                const attemptInit = (retries = 0) => {
                    if ($el.find('option').length > 0 || retries > 10) {
                        initSelect2($el, property);
                    } else {
                        setTimeout(() => attemptInit(retries + 1), 80);
                    }
                };
    ...
    // Re-initialize after Livewire morphs DOM
    Livewire.hook('morph.updated', ({ el }) => {
        ...
        }, 100); // Slight delay to let DOM settle before re-init
    });

    // Manual debug helper
    window.forceReinitSelect2 = () => initAllSelect2();
