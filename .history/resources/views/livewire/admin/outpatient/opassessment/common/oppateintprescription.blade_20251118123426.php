<div class="col-md-12">
    ...
    <!-- LOADING DROPDOWN -->
    <ul class="dropdown-menu show w-100 mt-1"
        style="display: none;"
        wire:loading
    ...
    @if(!empty($searchquery))
        <ul class="dropdown-menu show w-100 mt-1 p-0 shadow-sm" style="max-height: 250px; overflow-y: auto;">
            @if(!empty($pharmacyproduct))
                @foreach ($pharmacyproduct as $eachpharmacyproduct)
                    <li class="list-group-item list-group-item-action"
                        wire:click="additem({{ $eachpharmacyproduct['id'] }})"
                        style="cursor: pointer;">
                        <div class="d-flex justify-content-between">
                            <strong>{{ $eachpharmacyproduct['name'] }}</strong>
                            <span class="badge bg-primary">SKU: {{ $eachpharmacyproduct['product_sku'] }}</span>
                        </div>
                    </li>
                @endforeach
            @else
