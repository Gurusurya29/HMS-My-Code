            ...
            <div class="row g-2">
                <!-- <div class="dropdown">
                    <input
                        ...
                        placeholder="Search Drug..."
                          wire:model.lazy.debounce.150ms="searchquery" />

                    <ul
                        class="dropdown-menu list-group w-100 mt-1"
                        style="display: {{ strlen($searchquery ?? '') > 0 ? 'block' : 'none' }};"
                ...
                <div class="dropdown w-100">
                    ...
                    <input
                        ...
                        class="form-control shadow-sm"
                        placeholder="Search Drug..."
                        wire:model.debounce.150ms="searchquery" />

                    <!-- LOADING DROPDOWN (Searching...) -->
                    @if(strlen($searchquery) > 0)
                    <ul class="dropdown-menu w-100 mt-1 d-block"
                        wire:loading
                        wire:target="searchquery">
                    ...
                    <ul class="dropdown-menu w-100 mt-1 d-block"
                        ...
                        @if(!empty($pharmacyproduct))
                        @foreach($pharmacyproduct as $eachpharmacyproduct)
                        <li wire:click="additem({{ $eachpharmacyproduct['id'] }})"
                            class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">

                            <h6 class="m-0">{{ $eachpharmacyproduct['name'] }}</h6>

                            <span class="badge bg-primary rounded-pill">
                                SKU: {{ $eachpharmacyproduct['product_sku'] }}
                            </span>
                        </li>
                        @endforeach
                        @else
