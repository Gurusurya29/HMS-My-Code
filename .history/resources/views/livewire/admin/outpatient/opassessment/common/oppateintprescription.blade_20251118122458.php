<div class="col-md-12">
    ...
    <input type="text"
        ...
        wire:model.debounce.250ms="searchquery"
        autocomplete="off">
    ...
    @if(strlen($searchquery) > 0)
        <ul class="dropdown-menu show w-100 mt-1 p-0" 
            wire:loading.remove 
            wire:target="searchquery">

            @forelse($pharmacyproduct as $drug)
                <li class="list-group-item list-group-item-action d-flex justify-content-between"
                    wire:click="additem({{ $drug['id'] }})">

                    <span>{{ $drug['name'] }}</span>

                    <span class="badge bg-primary">SKU: {{ $drug['product_sku'] }}</span>
                </li>
            @empty
                <li class="list-group-item d-flex justify-content-between">
                    No results <span class="badge bg-danger">0</span>
                </li>
            @endforelse
        </ul>
    @endif
