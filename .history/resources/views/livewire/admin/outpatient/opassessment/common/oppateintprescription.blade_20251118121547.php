<div class="col-md-12">
    ...
    <input
        ...
        wire:model.livedebounce.150ms="searchquery"
    />
    ...
    @if(strlen($searchquery) > 0)
        <ul class="dropdown-menu list-group w-100 mt-1 d-block"
            ...
            @if(!empty($pharmacyproduct))
                @foreach($pharmacyproduct as $each)
                    <li
                        class="list-group-item list-group-item-action d-flex justify-content-between align-items-center"
                        wire:click="additem({{ $each['id'] }})"
                    >
                        <h6 class="m-0">{{ $each['name'] }}</h6>
                        <span class="badge bg-primary rounded-pill">SKU: {{ $each['product_sku'] }}</span>
                    </li>
                @endforeach
            @else
                <li class="list-group-item d-flex justify-content-between align-items-center">
