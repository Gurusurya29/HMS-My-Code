...
public $firstClickOccurred = false;
    ...
    public function show($emr_id, $showdatatype)
    {
        ...
        if (!$this->firstClickOccurred) {
            // This is the first click globally
            $this->firstClickOccurred = true; // Set the flag for next time
            // Do NOT dispatch the 'showmodal' event
            // You can add a log or flash message here if needed
            session()->flash('info', 'View details activated. Click again to see the modal.');
            return; 
        }

        // This is the second click or later
        $this->showdatatype = $showdatatype;
        $this->showdata = Emr::find($emr_id)->emrable;
        
        // 3. Dispatch the event only on subsequent clicks
        $this->dispatch('showmodal');
    }
