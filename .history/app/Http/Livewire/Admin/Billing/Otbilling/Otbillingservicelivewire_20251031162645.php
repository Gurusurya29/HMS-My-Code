    ...
    public function storeservice()
{
    ...
    try {
        ...
        foreach ($this->selectedotservicemasterlist as $key => $value) {
            ...
            // ✅ Check if service already exists for the same OT Billing
            $existingService = $this->user->otbillingservicelistcreatable()
                ->where('otbilling_id', $value['otbilling_id'])
                ->where('ipservicemaster_id', $value['ipservicemaster_id'])
                ->first();

            if ($existingService) {
                // If already exists, update the quantity & amount
                $newQuantity = $existingService->quantity + intval($value['quantity']);
                $newFinalAmount = doubleval($value['otservice_fee']) * $newQuantity;
        ...
        // ✅ Update OT Billing Totals
        $subtotal = $this->otbillingdata->otbillingservicelist()->sum('final_amount');
        $grandtotal = $subtotal - $this->otbillingdata->discount;
        ...
        $this->toaster('success', 'OT Billing Service list Created/Updated Successfully!!');
        return redirect()->route('otbillingservice', $this->otbillingdata->uuid);
    ...
    public function render()
    {
        ...
        return view('livewire.admin.billing.otbilling.otbillingservicelivewire',
