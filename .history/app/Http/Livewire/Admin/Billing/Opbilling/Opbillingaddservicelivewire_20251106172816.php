...
class Opbillingaddservicelivewire extends Component
{
    ...
    public $user, $balance;
    public $showdata, $opbillingdata, $opbillinglistdata, $opservicemasterlist, $searchquery, $selectedopservicemasterlist = [];
    public $discount = 0, $totalbillvalue = 0, $grandtotalvalue = 0;

    #[On('addserviceresetfields')]
    ...
    protected $rules = [
        ...
        'selectedopservicemasterlist.*.opservice_fee' => 'required|numeric',
        'selectedopservicemasterlist.*.quantity' => 'required|integer|min:1',
        'selectedopservicemasterlist.*.final_amount' => 'required|numeric|min:0',
    ];
    ...
    protected $messages = [
        ...
        'selectedopservicemasterlist.*.opservice_fee.numeric' => 'Enter valid value.',
    ...
    {
        $this->opbillingdata = Opbilling::where('uuid', $opbilling_uuid)->firstOrFail();
    ...
    public function updatedSearchquery()
    {
        $this->opservicemasterlist = Opservicemaster::where('active', true)
            ->whereNotIn('id', collect($this->selectedopservicemasterlist)
                ->whereNotNull('opservicemaster_id')
                ->pluck('opservicemaster_id')
                ->toArray()
            )
    ...
    {
        $opservicefee = $this->opbillingdata->patientvisit->billing_type == 1
            ? $opservicemaster->selffee
            : $opservicemaster->insurancefee;
    ...
    public function billingservicecalc($key)
    {
        ...
        $this->selectedopservicemasterlist[$key]['final_amount'] =
            doubleval($opservicemasterlist['opservice_fee']) * intval($opservicemasterlist['quantity']);
        $this->calculateTotals();
    }
    ...
    {
        $this->validate([
            'discount' => 'nullable|numeric|min:0|lte:' . $this->totalbillvalue,
        ], [
            'discount.numeric' => 'Enter a valid discount amount.',
        ]);
    ...
    private function calculateTotals()
    {
        ...
        $discount = min($this->discount ?? 0, $total);
    ...
    public function storeservice()
    {
        ...
        $this->validate([
            'totalbillvalue' => 'required|numeric|min:0',
            'discount' => 'nullable|numeric|min:0|lte:' . $this->totalbillvalue,
            'grandtotalvalue' => 'required|numeric|min:0',
        ]);
        ...
        try {
            ...
            $opbillinglist = $this->user->opbillinglistcreatable()->create([
                'patient_id' => $this->opbillingdata->patient_id,
                'opbilling_id' => $this->opbillingdata->id,
                'sub_total' => $this->totalbillvalue,
                'discount' => $this->discount,
                'total' => $this->grandtotalvalue,
                'billdiscount_amount' => 0,
                'grand_total' => $this->grandtotalvalue,
            ]);

            foreach ($this->selectedopservicemasterlist as $value) {
                $this->user->opbillingservicelistcreatable()->create([
                    ...
                    'opservice_selffee' => $value['opservice_selffee'] ?: $value['opservice_fee'],
                    'opservice_insurancefee' => $value['opservice_insurancefee'] ?: $value['opservice_fee'],
            ...
            $this->dispatch('toaster', type: 'success', message: 'OP Billing Service list Created Successfully!');
            $this->selectedopservicemasterlist = [];
    ...
    {
        $pdf = Pdf::loadView('livewire.admin.billing.opbilling.opbillinglistpdf', compact('opbillinglist'))
    ...
    public function render()
    {
        ...
        return view('livewire.admin.billing.opbilling.opbillingaddservicelivewire', [
            'total' => $this->totalbillvalue,
            'discount' => $this->discount,
            'grandtotal' => $this->grandtotalvalue,
        ]);
    }
}
