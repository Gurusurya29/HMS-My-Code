...
class Opbillingaddservicelivewire extends Component
{
    ...
    public function updatedDiscount()
    {
        // Validate discount as numeric and non-negative
        $this->validate([
            'discount' => 'nullable|numeric|min:0',
        ], [
            'discount.numeric' => 'Please enter a valid discount amount.',
        ]);

        // Ensure it's not negative
        $this->discount = max(0, floatval($this->discount));
    ...
    {
        if (!empty($this->selectedopservicemasterlist)) {
            $finalamounts = collect($this->selectedopservicemasterlist)->pluck('final_amount')->toArray();
            $total = array_sum($finalamounts);

            // Apply discount but prevent it from exceeding total
