...
class Opbillingaddservicelivewire extends Component
{
    ...
    public function updatedDiscount()
    {
        // Force cast and validate discount so arithmetic works
        $this->discount = $this->discount === '' ? 0 : floatval($this->discount);

        $this->validate([
            'discount' => 'nullable|numeric|min:0|lte:' . ($this->totalbillvalue ?? 0),
        ], [
            'discount.numeric' => 'Enter valid value.',
            'discount.lte' => 'Discount cannot exceed total.',
        ]);

        // ensure discount never exceeds total in code (defensive)
        if ($this->discount > ($this->totalbillvalue ?? 0)) {
            $this->discount = floatval($this->totalbillvalue ?? 0);
        }
    }
    ...
    {
        if (!empty($this->selectedopservicemasterlist)) {
            $finalamounts = collect($this->selectedopservicemasterlist)->pluck('final_amount')->map(function ($v) {
                return doubleval($v);
            })->toArray();

            $total = array_sum($finalamounts);
            // ensure discount is numeric
            $discount = $this->discount === '' ? 0 : floatval($this->discount);
            // Do not treat 0 as "false" — explicitly subtract
            ...
            $total = 0;
            $grandtotal = 0;
            // keep discount at 0 when nothing selected
            $this->discount = 0;
        }

        $this->totalbillvalue = floatval($total);
        $this->grandtotalvalue = floatval($grandtotal);
