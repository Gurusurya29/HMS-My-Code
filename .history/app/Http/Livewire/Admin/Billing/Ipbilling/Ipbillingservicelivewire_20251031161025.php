...
class Ipbillingservicelivewire extends Component
{
    ...
    public function storeservice()
    {
        ...
        try {
            ...
            foreach ($this->selectedipservicemasterlist as $key => $value) {
                ...
                // 🛑 Check for duplicate entry (same ipbilling_id + ipservicemaster_id)
                $exists = $this->user->ipbillingservicelistcreatable()
                    ->where('ipbilling_id', $value['ipbilling_id'])
                    ->where('ipservicemaster_id', $value['ipservicemaster_id'])
                    ->exists();
                ...
                // ✅ Create new billing service record
                $ipbillingservicelist = $this->user->ipbillingservicelistcreatable()->create([
                    'patient_id' => $value['patient_id'],
                    'inpatient_id' => $value['inpatient_id'],
                    'ipbilling_id' => $value['ipbilling_id'],
                    'ipadmission_id' => $value['ipadmission_id'],
                    ...
                    'ipservice_name' => $value['ipservice_name'],
                    'ipservice_fee' => $value['ipservice_fee'],
                    'ipservice_selffee' => $value['ipservice_selffee'] != 0
                        ? $value['ipservice_selffee']
                        : $value['ipservice_fee'],
                    'ipservice_insurancefee' => $value['ipservice_insurancefee'] != 0
                        ? $value['ipservice_insurancefee']
                        : $value['ipservice_fee'],
                    'quantity' => $value['quantity'],
                    'final_amount' => $value['final_amount'],
                ]);
                ...
                $this->user->patientstatementcreatable()->make([
                    ...
                    'credit' => 0,
                    'debit' => $value['final_amount'],
                    'self_fee' => $value['ipservice_selffee'] != 0
                        ? (doubleval($value['ipservice_selffee']) * intval($value['quantity']))
                        : $value['final_amount'],
                    'insurance_fee' => $value['ipservice_insurancefee'] != 0
                        ? (doubleval($value['ipservice_insurancefee']) * intval($value['quantity']))
                        : $value['final_amount'],
                    'note' => 'IP Bill Service',
                    'entity_type' => 1,
                    'transaction_type' => 'D',
                ...
                Helper::trackmessage(
                    $this->user,
                    $ipbillingservicelist,
                    'ipbillingservicelistcreateoredit',
                    session()->getId(),
                    'WEB',
                    'IP Billing Service list Created'
                );
            }
    ...
    public function render()
    {
        ...
        return view(
            'livewire.admin.billing.ipbilling.ipbillingservicelivewire',
