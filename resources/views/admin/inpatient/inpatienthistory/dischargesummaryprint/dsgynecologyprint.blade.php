...
<body>
    ...
    <div id="printdiv">
        ...
        @endif
        @if ($inpatient->dsspecialable->progress)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">PROCEDURE:</div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->progress !!}</p>
        @endif
        @if ($inpatient->dsspecialable->natureofdelivery)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">NATURE OF DELIVERY:</div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->natureofdelivery !!}</p>
        @endif
        @if ($inpatient->dsspecialable->pih)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">PIH:</div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->pih !!}</p>
        @endif
        @if ($inpatient->dsspecialable->postnatalperiod)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">POST-NATAL PERIOD:</div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->postnatalperiod !!}</p>
        @endif
        @if ($inpatient->dsspecialable->medicinesgiven)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">MEDICINES GIVEN:</div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->medicinesgiven !!}</p>
        @endif
        @if ($inpatient->dsspecialable->babydetail)
            @php
                $babydetail;
                $babydetailarr = [];
                foreach (json_decode($inpatient->dsspecialable->babydetail) as $key => $value) {
                    array_push($babydetailarr, [
                        'gender' => $value->gender,
                        'weight' => $value->weight,
                        'apgar' => $value->apgar,
                        'dateandtime' => $value->dateandtime ? date('d-m-Y h:i A', strtotime(date('d-m-Y h:i A', strtotime($value->dateandtime)))) : '',
                    ]);
                }
                $babydetail = $babydetailarr;
            @endphp
            @if ($babydetail[0]['dateandtime'] != '')
                <div style="text-align: center; font-weight: bold;font-size:25px; margin-top:3%">SUMMARY ABOUT YOUR SON
                    /
                    DAUGHTER</div>
                <table style="width:100%; margin-top:3%;margin-bottom:3%;text-align:center" class="data">
                    ...
                    <tbody>
                        @foreach ($babydetail as $eachbabydetail)
                            <tr>
                                <td style="width:25%;">{{ $eachbabydetail['gender'] }}</td>
                                <td style="width:25%;">
                                    {{ $eachbabydetail['weight'] }}</td>
                                <td style="width:25%;">{{ $eachbabydetail['apgar'] }}</td>
                                <td style="width:25%;">{{ $eachbabydetail['dateandtime'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @endif
        @endif
        @if ($inpatient->dsspecialable?->dsgynecologybaby?->babyfull_detail)
            <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%">BABY DETAILS:
            </div>
            <p style="margin-left: 7%">{!! $inpatient->dsspecialable->dsgynecologybaby->babyfull_detail !!}</p>
        @endif
        ...
        @if ($inpatient->dsspecialable->followupvisit)
            ...
            @if ($followupvisit[0]['scheduledate'] != '')
                <div style="text-align: left; font-weight: bold;font-size:16px; margin-top:3%;">FOLLOW UP
                    VISITS</div>
                <table style="width:100%; margin-top:3%;margin-bottom:3%;text-align:center;" class="data">
                    <thead>
                        <th>
