    ...
    <div class="card shadow-sm">
        <div class="card-header text-white theme_bg_color p-1">
            <div class="d-flex flex-row bd-highlight">
                <div class="flex-grow-1 bd-highlight mt-1"><span class="h5">COMPLETED SURGERY REPORT</span>
                </div>
                ...
                <table id="patientreport_id" class="table text-center table-hover m-0 p-0">
                    ...
                    <tbody>
                        @foreach ($otschedulelist as $key => $item)
                            <tr>
                                <td>{{ $otschedulelist->firstItem() + $key }}</td>
                                <td>{{ $item->patient?->uhid }}</td>
                                <td>{{ $item->patient?->name }}</td>
                                <td>{{ $item->patient?->phone }}</td>
                                <td>{{ $item->doctor?->name }}</td>
                                <td>{{ $item->surgery_name }}</td>
                                <td>{{ Carbon\Carbon::parse($item->surgery_startdate)->format('d-m-Y h:i A') }}</td>
                                <td>{{ Carbon\Carbon::parse($item->surgery_enddate)->format('d-m-Y h:i A') }}</td>
