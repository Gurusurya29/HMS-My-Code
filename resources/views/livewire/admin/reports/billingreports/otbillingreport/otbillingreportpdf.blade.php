    ...
    <table style="border: 1px solid; border-collapse: collapse; width:100%; font-size:10px;">
        <thead>
            <tr>
                ...
                <th style="border: 1px solid;">IP ID</th>
        ...
        <tbody>
            @foreach ($otbilling as $key => $item)
                <tr style="text-align:center;">
                    ...
                    <td style="border: 1px solid;">{{ $item->inpatient->uniqid }}</td>
                    <td style="border: 1px solid;">{{ $item->uniqid }}</td>
                    <td style="border: 1px solid;">
                        {{ $item->updatable_id ? $item->updatable->name : $item->creatable->name }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $otbilling->sum('sub_total') }}</td>
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $otbilling->sum('discount') }}</td>
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $otbilling->sum('total') }}</td>
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $otbilling->sum('billdiscount_amount') }}</td>
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $otbilling->sum('grand_total') }}</td>
            </tr>
        </tfoot>
    </table>
</div>
