    ...
    <div class="card shadow-sm">
        <div class="card-header text-white theme_bg_color p-1">
            <div class="d-flex flex-row bd-highlight">
                <div class="flex-grow-1 bd-highlight mt-1"><span class="h5">SUPPLIER LEDGER REPORT</span>
                </div>
        ...
        <div class="card-body p-0 row justify-content-center">
            <div class="my-3 col-md-8">
                <div class="dropdown">
                    <label class="form-label fs-6" for="searchquery">Search Supplier :</label>
                    <input type="text" class="form-control form-control-sm shadow-sm border border-1 border-info"
                        id="searchquery" placeholder="Search Supplier..." wire:model="searchquery"   wire:model.live.debounce.300ms="searchquery" />
                    ...
                    @if (!empty($searchquery))
                        <ul class="dropdown-menu list-group w-100 p-0">
                            @if (!empty($supplierlist))
                                @foreach ($supplierlist as $i => $eachsupplierlist)
                                    <li wire:click="selectedsupplier({{ $eachsupplierlist['id'] }})"
                                        class="list-group-item list-group-item-action d-flex justify-content-between align-items-center"
                                        role="button">
                                        <h6> {{ $eachsupplierlist['company_name'] }} </h6>

                                        <h5>
                                            <span class=" badge bg-primary rounded-pill">
                                                {{ $eachsupplierlist['uniqid'] }}</span>
                                        </h5>
                                    </li>
                                @endforeach
                            @else
            ...
            @if ($supplier)
                <div class="table-responsive col-md-8">
                    <table class="table table-bordered border-secondary shadow-sm table-success text-center">
                        ...
                        <tbody>
                            <tr class="fs-5">
                                <td>{{ $supplier->uniqid ?? '-' }}</td>
                                <td>{{ $supplier->company_name ?? '-' }}</td>
                                <td>{{ $supplier->contact_phone_no ?? '-' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="table-responsive">
                    <table id="supplierstatementreport_id" class="table text-center table-hover m-0 p-0">
                        ...
                        <tbody>
                            @foreach ($supplierstatement as $key => $item)
                                <tr>
                                    <td>{{ $supplierstatement->firstItem() + $key }}</td>
