    ...
    <div class="card shadow-sm">
        <div class="card-header text-white theme_bg_color p-1">
            <div class="d-flex flex-row bd-highlight">
                <div class="flex-grow-1 bd-highlight mt-1"><span class="h5">LOGIN LOGS REPORT</span>
                </div>
                ...
                <table id="patientreport_id" class="table text-center table-hover m-0 p-0">
                    <thead class="text-white theme_bg_color">
                        <tr>
                            ...
                            <th>UNIQID</th>
                            <th>NAME</th>
                            <th>DEVICE / BROWSER / PLATFORM / TYPE</th>
                            <th>STATUS</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($loginlogs as $key => $item)
                            <tr>
                                <td>{{ $loginlogs->firstItem() + $key }}</td>
                                <td>{{ $item->created_at->format('d-m-Y h:i A') }}</td>
                                <td>{{ $item->logininfoable->uniqid }}</td>
                                <td>{{ $item->logininfoable->name }}
                                    ({{ ucwords(strtolower($item->logininfoable->usertype)) }})
                                </td>
                                <td>{{ $item->device }} / {{ $item->browser }} /
                                    {{ $item->platform }} / {{ $item->type }}</td>
                                <td>{{ $item->login_status ? 'Success' : 'Failed' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
