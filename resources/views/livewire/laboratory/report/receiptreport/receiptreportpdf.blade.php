<div>
    ...
    <table style="border: 1px solid; border-collapse: collapse; width:100%; font-size:10px;">
        ...
        <tbody>
            @foreach ($receiptreport as $key => $item)
                <tr>
        ...
        <tfoot>
            <tr>
                ...
                <td style="border: 1px solid;text-align:center; padding:5px;">
                    {{ $receiptreport->sum('received_amount') }}</td>
            </tr>
        </tfoot>
    </table>
    <table style="border: 1px solid; border-collapse: collapse; width:100%; font-size:10px; margin-top:15px;">
        ...
        <tbody>
            ...
            <tr style="text-align:center;">
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 1)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 2)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 3)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 4)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 5)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 6)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 7)->sum('received_amount') }}
                </td>
                <td style="border: 1px solid;">{{ $receiptreport->where('modeofpayment', 8)->sum('received_amount') }}
                </td>
            </tr>
        </tbody>
    </table>
