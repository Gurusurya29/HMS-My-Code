                            ...
                            <div class="mx-auto">
                                <div class="dropdown">
                                    <label for="searchqueryid" class="col-form-label fw-bold">SEARCH
                                        INVESTIGATION</label>
                                    <input type="text" class="form-control shadow-sm bg-white" id="searchqueryid"
                                        placeholder="Search Investigation..." wire:model="searchquery"   wire:model.live.debounce.300ms="searchquery" />

                                    {{-- <div class="dropdown-menu list-group w-100">
                                    <span
                                        class="list-group-item d-flex justify-content-between align-items-center">Searching...</span>
                                </div> --}}

                                    @if (!empty($searchquery))
                                        <ul class="dropdown-menu list-group w-100 border-0 m-0 p-0">
                                            @if (!empty($labinvestigation))
                                                @foreach ($labinvestigation as $i => $eachlabinvestigation)
                                                    <li wire:click="additem({{ $eachlabinvestigation['id'] }})"
                                                        class="list-group-item list-group-item-action d-flex justify-content-between align-items-center"
                                                        role="button">
                                                        <h6> {{ $eachlabinvestigation['name'] }} </h6>
                                                    </li>
                                                @endforeach
                                            @else
                                                <li
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped p-0 m-0">
                                        ...
                                        <tbody>
                                            @if ($selectedlabinvestigation)
                                                @foreach ($selectedlabinvestigation as $key => $eachlabinvestigation)
                                                    <tr>
                                                        <td>
                                                            {{ $key + 1 }}
                                                        </td>
                                                        <td>
                                                            {{ $eachlabinvestigation['labinvestigation_name'] }}
                                                        </td>
                                                        <td>
                                                            {{ $eachlabinvestigation['selffee'] }}
                                                        </td>
                                ...
                                @if (!empty($selectedlabinvestigation))
