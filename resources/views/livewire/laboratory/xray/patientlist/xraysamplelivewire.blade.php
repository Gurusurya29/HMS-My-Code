            ...
            <table class="table table-bordered  p-0 m-0 text-center">
                <thead class="theme_bg_color text-white">
                    <tr>
                        ...
                        <th class="w-25">X-RAY DONE</th>
                        <th class="w-50">NOTES</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($xraypatient->xraypatientlist->where('is_movedtobill', true) as $key => $eachxraysample)
                        <tr>
                            <td class="{{ $eachxraysample->is_sampletaken ? 'text-success' : '' }}">
                                {{ $key + 1 }}</td>
                            <td class="w-50 {{ $eachxraysample->is_sampletaken ? 'text-success' : '' }} fs-5">
                                {{ $eachxraysample->xrayinvestigation_name }}</td>
                            <td class="w-25">
                                <div class="form-check form-switch form-switch-md d-flex justify-content-center">
                                    <input wire:click="marksampledone({{ $eachxraysample->id }})"
                                        {{ $eachxraysample->is_sampletaken ? 'checked' : '' }} class="form-check-input"
                            ...
                            <td class="w-25">
                                <div class="m-0 p-0">
                                    ...
                                    <textarea class="form-control " wire:model.debounce.150ms="sample_note.{{ $eachxraysample->id }}" rows="1"></textarea>
                                </div>
                            </td>
                        </tr>
                    @endforeach
