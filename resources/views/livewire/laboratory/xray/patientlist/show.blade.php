        ...
        @if ($showdata)
            <div class="modal-content">
                <div class="modal-header text-white theme_bg_color px-3 py-2">
                    <h5 class="modal-title" id="showModalLabel">X-RAY INVESTIGATION DETAILS : {{ $showdata->uniqid }}
                    </h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        @include('helper.formhelper.showlabel', [
                            'name' => 'TOTAL',
                            'value' => $showdata->xraypatientlist->sum('fee'),
                            'columnone' => 'col-md-6',
                            'columntwo' => 'col-5',
                            'columnthree' => 'col-7',
                        ])
                ...
                <div class="card-body">
                    <div class="table-responsive shadow-sm mb-2">
                        <table class="table table-bordered  p-0 m-0">
                            ...
                            <tbody>
                                @foreach ($showdata->xraypatientlist as $key => $eachxraylist)
                                    <tr class="text-center">
                                        <td class="{{ $eachxraylist->is_resultupdated ? 'text-success' : '' }}">
                                            {{ $key + 1 }}</td>
                                        <td class="{{ $eachxraylist->is_resultupdated ? 'text-success' : '' }}">
                                            {{ $eachxraylist->xrayinvestigationgroup_name }}</td>
                                        <td class="{{ $eachxraylist->is_resultupdated ? 'text-success' : '' }}">
                                            {{ $eachxraylist->xrayinvestigation_name }}</td>

                                        <td>
                                            {{ $eachxraylist->is_movedtobill ? 'Yes' : 'No' }}
                                        </td>
                                        <td>
                                            {{ $eachxraylist->is_sampletaken ? 'Yes' : 'No' }}
                                        </td>

                                        <td>
                                            {{ $eachxraylist->is_resultupdated ? 'Yes' : 'No' }}
                                        </td>

                                        <td>{{ $eachxraylist->fee }}</td>
                                        <td>
                                            <button class="accordion-button collapsed p-1" type="button"
                                                data-bs-toggle="collapse" data-bs-target="#xray_{{ $key }}"
                                                aria-expanded="true" aria-controls="xray_{{ $key }}">More
                                            </button>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="9" class="p-0 table-light">
                                            <div id="xray_{{ $key }}" class="accordion-collapse collapse">
                                                <div class="row px-3">
                                                    @include('helper.formhelper.showlabel', [
                                                        'name' => 'UNITS',
                                                        'value' => $eachxraylist->units,
                                                        'columnone' => 'col-md-6',
                                                        'columntwo' => 'col-5',
                                                        'columnthree' => 'col-7',
                                                    ])
                                                    ...
                                                    @include('helper.formhelper.showlabel', [
                                                        'name' => 'SAMPLE NOTE',
                                                        'value' => $eachxraylist->sample_note,
                                                        'columnone' => 'col-md-6',
                                                        'columntwo' => 'col-5',
                                                        'columnthree' => 'col-7',
                                                    ])
                                                    @include('helper.formhelper.showlabel', [
                                                        'name' => 'RESULT VALUE',
                                                        'value' => $eachxraylist->result_note,
                                                        'columnone' => 'col-md-6',
                                                        'columntwo' => 'col-5',
                                                        'columnthree' => 'col-7',
                                                    ])
                                                    @if ($eachxraylist->xray_image)
                                                        <div class="row col-md-6 p-1">
                                                            ...
                                                            <div class="col-7">
                                                                <b> : </b>
                                                                <button
                                                                    wire:click="downloadFile('{{ $showdata->uniqid }}','{{ $eachxraylist->xray_image }}')"
                                                                    type="button" class="btn btn-sm btn-success"><i
                                                                        class="bi bi-download"></i></button>
                                                            </div>
                                                        </div>
                                                    @endif
                                                    @if ($eachxraylist->is_senttoexternallab)
                                                        @include('helper.formhelper.showlabel', [
                                                            'name' => 'EXTERNAL LAB NOTE',
                                                            'value' => $eachxraylist->senttoexternal_note,
                                                            'columnone' => 'col-md-6',
                                                            'columntwo' => 'col-5',
                                                            'columnthree' => 'col-7',
                                                        ])
                                ...
                                {{-- <tr>
                                    ...
                                    <td>{{ $showdata->xraypatientlist->sum('fee') }}</td>
                                </tr> --}}
                            </tbody>
                        </table>
                    </div>
